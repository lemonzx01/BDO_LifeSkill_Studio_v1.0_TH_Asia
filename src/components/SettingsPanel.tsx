...
import type { RecipeType, Settings } from "@/lib/engine/types";
import { netRate } from "@/lib/engine/cost";
import { MASTERY_MAX, maxQuantityChance } from "@/lib/engine/mastery";
import { pct } from "@/lib/format";

const TIERS = ["มือใหม่", "ฝึกฝน", "คล่องแคล่ว", "เชี่ยวชาญ", "ช่าง", "ลือชื่อ", "เซียน"];
const SKILLS: { key: RecipeType; label: string }[] = [
        ...
        <div className="space-y-2 text-sm">
          ...
          <label className="flex items-center justify-between gap-3">
            ...
            <select
              ...
              onChange={(e) => set({ familyFame: Number(e.target.value) })}
        ...
        <div className="space-y-3 text-sm">
          {SKILLS.map(({ key, label }) => (
            <div key={key} className="grid grid-cols-[auto_1fr_1fr_1fr] items-center gap-2">
              ...
              <label className="flex flex-col gap-1 text-xs text-muted">
                Mastery (โอกาสได้เต็ม {pct(maxQuantityChance(key, settings.mastery[key] ?? 0), 1)})
                <input
                  type="number"
                  step="50"
                  min="0"
                  max={MASTERY_MAX}
                  value={settings.mastery[key] ?? 0}
                  onChange={(e) =>
                    set({ mastery: { ...settings.mastery, [key]: Math.max(0, Math.min(MASTERY_MAX, Number(e.target.value) || 0)) } })
                  }
          ...
          <p className="text-xs text-muted">
            ใส่ค่า Mastery ในเกม ระบบจะคิดโอกาสได้ผลผลิตจำนวนสูงสุดต่อรอบจากตารางของเกมให้ (เช่น สูตร 1~4 ชิ้น ที่ Mastery 2000 แปรธาตุได้เฉลี่ย 3.25 ชิ้น/รอบ)
          </p>
        </div>
      </section>
    </div>
